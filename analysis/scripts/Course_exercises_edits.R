...
#export as csv
rio::export(data_Jose, "analysis/data/data_Jose_March2024test.csv")
#export as compressed csv
rio::export(data_Jose, "analysis/data/data_Jose_March2024test.csv.zip")
...
plot_Jose1 <- data_Jose %>%
  ...
  geom_boxplot() +
  theme_minimal() +
  scale_fill_manual(values = c("#D55E00", "#E69F00", "#e55f99")) +
...
ggsave( "analysis/pictures/plot_Jose1a.jpg",
  limitsize = FALSE,
  units = c("px"), plot_Jose1,
  width = 3400, height = 2600
  )

# save in a different size
ggsave( "analysis/pictures/plot_Jose1b.jpg",
  limitsize = FALSE,
...
ggsave(
  "analysis/pictures/synuclein_plot.jpg", plot_syn, 
  bg = "white"
  )
...
img3 <- readPNG("analysis/pictures/synuclein_plot.png")

#convert to panels
panel_JoseA <- ggdraw() + draw_image(img1)
panel_JoseB <- ggdraw() + draw_image(img2)
panel_syn <- ggdraw() + draw_image(img3)
...
Figure_Jose <- panel_JoseA + panel_JoseB + panel_syn +
...
panel_NOS2d_HCR <- ggdraw() + draw_image(readPNG("analysis/pictures/HCR-IHC_51_AP_NOS_actub_56um.png")) +
  draw_label("in situ HCR", x = 0.25, y = 0.99, size = 10) +
